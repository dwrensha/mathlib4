...
-- Attribute that gets added to every declaration generated by an alias command.
initialize aliasTag : TagAttribute ←
...
| `(alias $name:ident ← $aliases:ident*) => do
  ...
  for a in aliases do
    let newName := (appendNamespace ns a.getId)
    ...
    Lean.Elab.Command.liftTermElabM none do
      aliasTag.attr.add newName Syntax.missing AttributeKind.global
...
def aliasIff (ci : ConstantInfo) (al : Name) (isForward : Bool) : MetaM Unit := do
  ...
  aliasTag.attr.add al Syntax.missing AttributeKind.global
